...
def write_summary(sample_cluster: pd.DataFrame, group_summary: pd.DataFrame, tests: pd.DataFrame, matrix: pd.DataFrame) -> None:
    ...
    lines = [
        ...
        "- Encoded groups are retained as `C_EM`, `AM_EuM`, and `AM_EcM`; check source metadata before interpretation.",
        ...
        f"- Candidate-context matrix rows: {len(matrix)}.",
        f"- Localization class counts: `{class_counts}`",
        "- Context score range: 0-25 from detectability, sample support, fibrovascular context, encoded-group spread and descriptive effect-size bonus.",
